...
title('LDA-Visualize the projected data vector in 2d');
...
title('LDA-Visualize the eigenvector1-2D');
subplot(1, 2, 2);
engienvector2 = reshape(engienvector_2d(:, 2), 28, 28);
imshow(engienvector2, []);
title('LDA-Visualize the eigenvector2-2D');

%% ========calculate the accuracy rate of test_images ======== %%
%reduce the dimension to 2D for train_images & test_images
for m = 1 : 2
    engienvector_2d(:, m) = eigenvector(:, index1(1, m));
end
y_train_2d = (engienvector_2d)' * train_images;
y_test_2d = (engienvector_2d)' * test_images;

%calculate the Euclidean distance for test_images
for column_test = 1:10000
    subtract_test_2d = repmat(y_test_2d(:, column_test), 1, 60000);
    dis_matrix = y_train_2d - subtract_test_2d;
...
acc_rate_2d_test = (10000 - error) / 10000;
disp('LDA_accuracy_rate_2d_test: ')
disp(acc_rate_2d_test)